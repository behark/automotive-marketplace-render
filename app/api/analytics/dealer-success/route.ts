  ...
  try {
    ...
    if ((user as any).role !== "dealer" && (user as any).role !== "admin") {
      return NextResponse.json(
        { error: "Access denied - dealers only" },
        { status: 403 }
      );
    }
    ...
    const multiLocationPerformance = await getMultiLocationPerformance(
      user.id,
      startDate
    );
    ...
    const inventoryOptimization = await getInventoryOptimization(
      user.id,
      startDate
    );
  ...
  const locationPerformance = await Promise.all(
    locations.map(async (location) => {
      ...
      const totalViews = listings.reduce(
        (sum, l) => sum + (l.favorites.length * 3 + l.messages.length * 2),
        0
      );
      const totalMessages = listings.reduce(
        (sum, l) => sum + l.messages.length,
        0
      );
      const totalLeads = listings.reduce((sum, l) => sum + l.leads.length, 0);
      const revenue = soldListings.reduce(
        (sum, l) => sum + (l.soldPrice || l.price),
        0
      );
      ...
      return {
        ...
        conversionRate:
          totalViews > 0 ? Math.round((totalMessages / totalViews) * 100) : 0,
        saleConversionRate:
          location._count.id > 0
            ? Math.round((soldListings.length / location._count.id) * 100)
            : 0,
      };
    })
  );
  ...
  const rankedLocations = locationPerformance
    ...
    .map((location, index) => ({
      ...
      performance:
        index === 0
          ? "Më e mira"
          : index < locationPerformance.length / 2
          ? "Mbi mesatare"
          : "Nën mesatare",
    }));
  ...
  return {
    ...
    avgPerformance: {
      avgRevenue: Math.round(
        rankedLocations.reduce((sum, l) => sum + l.revenue, 0) /
          rankedLocations.length
      ),
      avgConversion: Math.round(
        rankedLocations.reduce((sum, l) => sum + l.conversionRate, 0) /
          rankedLocations.length
      ),
      avgSaleConversion: Math.round(
        rankedLocations.reduce((sum, l) => sum + l.saleConversionRate, 0) /
          rankedLocations.length
      ),
    },
  };
...
async function getLeadManagement(dealerId: string, startDate: Date) {
  ...
  const leadAnalysis = {
    ...
    avgLeadPrice:
      leads.length > 0
        ? Math.round(
            leads.reduce((sum, l) => sum + l.price, 0) / leads.length / 100
          )
        : 0,
    leadConversionRate:
      leads.length > 0
  ...
  return {
    leadAnalysis,
    leadsByCarType: Object.entries(leadsByCarType)
      .map(([carType, data]) => ({
        carType,
        ...data,
      }))
...
async function getInventoryOptimization(dealerId: string, startDate: Date) {
  ...
  > = {};
  const inventoryByYear: Record<
    string,
    { count: number; sold: number; avgPrice: number; conversionRate?: number }
  > = {};
  const inventoryByPriceRange: Record<
    string,
    { count: number; sold: number; conversionRate?: number }
  > = {};
  ...
  return {
    inventoryByMake: Object.entries(inventoryByMake)
      .map(([make, data]) => ({ make, ...data }))
      .sort((a, b) => b.count - a.count),
    inventoryByYear: Object.entries(inventoryByYear).map(([year, data]) => ({
      year,
      ...data,
    })),
    inventoryByPriceRange: Object.entries(inventoryByPriceRange).map(
      ([range, data]) => ({ range, ...data })
    ),
    slowMovingInventory: slowMovingInventory.sort(
      (a, b) => b.daysSinceCreated - a.daysSinceCreated
    ),
    albanianPreferences,
    restockingRecommendations: getRestockingRecommendations(
      inventoryByMake,
      inventoryByYear
    ),
...
async function getExpansionOpportunities(dealerId: string, startDate: Date) {
  ...
  const expansionOpportunities = albanianCities
    ...
    .map((city) => ({
      ...city,
      investmentLevel:
        city.marketPotential > 80
          ? "I lartë"
          : city.marketPotential > 60
          ? "Mesatar"
          : "I ulët",
      timeToROI:
        city.marketPotential > 80
  ...
  return {
    ...
    expansionStrategy: getExpansionStrategy(
      currentLocations,
      expansionOpportunities
    ),
...
async function getStaffPerformance(dealerId: string, startDate: Date) {
  ...
  const performanceMetrics = staffMembers.map((staff) => ({
    ...
    conversionRate: Math.round(
      (staff.salesClosed / staff.leadsGenerated) * 100
    ),
    avgRevenuePerSale: Math.round(
      staff.revenueGenerated / staff.salesClosed / 100
    ),
  ...
  const teamOverview = {
    ...
    totalRevenue: Math.round(
      staffMembers.reduce((sum, s) => sum + s.revenueGenerated, 0) / 100
    ),
    totalSales: staffMembers.reduce((sum, s) => sum + s.salesClosed, 0),
    avgSatisfaction:
      Math.round(
        (staffMembers.reduce((sum, s) => sum + s.customerSatisfaction, 0) /
          staffMembers.length) *
          10
      ) / 10,
    avgConversionRate: Math.round(
      (staffMembers.reduce(
        (sum, s) => sum + s.salesClosed / s.leadsGenerated,
        0
      ) /
        staffMembers.length) *
        100
...
async function getMarketingROI(dealerId: string, startDate: Date) {
  ...
  const albanianMarketingInsights = [
    {
      channel: "Facebook",
      insight:
        "Platforma më e përdorur në Shqipëri - 78% e popullsisë e përdor",
  ...
  const totalInvestment = marketingChannels.reduce(
...
function getRestockingRecommendations(
  inventoryByMake: any,
  inventoryByYear: any
) {
  return [
...
function calculateStaffPerformanceScore(staff: any) {
  ...
  const responseTimeScore = Math.max(
    0,
    100 - parseFloat(staff.avgResponseTime) * 10
  ); // Lower is better
...
function getBudgetRecommendations(channels: any[]) {
  const bestROI = channels.reduce((best, current) =>
    current.roi > best.roi ? current : best
  );
...
function getSeasonalMarketingCalendar() {
  return {
    spring: [
      "Panaire pranvere",
      "Promovim convertible",
      'Kampanja "Makina e re për sezonin e ri"',
    ],
    summer: [
      "Festival verore",
      "Sponsorship evenimente",
      "Promovim SUV për pushime",
    ],
    fall: [
      'Kampanja "Përgatituni për dimër"',
      "Trade-in promovim",
      "Financim special",
    ],
    winter: [
      "Promovim makina 4x4",
      "Oferta të fundit vitit",
      'Kampanja "Makina për Vitin e Ri"',
    ],
  };
}
