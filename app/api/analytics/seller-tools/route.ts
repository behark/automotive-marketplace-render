...
async function getPricingSuggestions(listingId: string | null, userId: string) {
  if (!listingId) {
    return NextResponse.json(
      { error: "Listing ID required for pricing analysis" },
      { status: 400 }
    );
  }
  ...
  const avgSimilarPrice =
    similarPrices.length > 0
      ? Math.round(
          similarPrices.reduce((sum, price) => sum + price, 0) /
            similarPrices.length
        )
      : 0;
  ...
  const seasonalRecommendedPrice = Math.round(
    recommendedPrice * seasonalAdjustment
  );
...
async function getOptimalTiming(listingId: string | null, userId: string) {
  ...
  const postingPatterns: Record<
    string,
    { count: number; avgMessages: number; avgFavorites: number }
  > = {};
  const viewPatterns: Record<string, any> = {};

  userListings.forEach((listing) => {
    const dayOfWeek = new Date(listing.createdAt).getDay();
    const hour = new Date(listing.createdAt).getHours();

    const dayName = ["Diel", "Hën", "Mar", "Mër", "Enj", "Pre", "Sht"][
      dayOfWeek
    ];
...
async function getCompetitorAnalysis(listingId: string | null, userId: string) {
  if (!listingId) {
    return NextResponse.json(
      { error: "Listing ID required for competitor analysis" },
      { status: 400 }
    );
  }
  ...
  const competitiveAnalysis = directCompetitors.map((comp) => {
    ...
    if (listing.price < comp.price) {
      advantages.push(
        `Çmim më i ulët (€${Math.round(listing.price / 100)} vs €${Math.round(
          comp.price / 100
        )})`
      );
    } else if (listing.price > comp.price) {
      disadvantages.push(
        `Çmim më i lartë (€${Math.round(listing.price / 100)} vs €${Math.round(
          comp.price / 100
        )})`
      );
    }
    ...
    if (listing.mileage < comp.mileage) {
      advantages.push(
        `Kilometra më pak (${listing.mileage.toLocaleString()} vs ${comp.mileage.toLocaleString()})`
      );
    } else if (listing.mileage > comp.mileage) {
      disadvantages.push(
        `Kilometra më shumë (${listing.mileage.toLocaleString()} vs ${comp.mileage.toLocaleString()})`
      );
    }
    ...
    return {
      ...
      disadvantages,
      activityLevel:
        compActivity > 5 ? "E lartë" : compActivity > 2 ? "Mesatare" : "E ulët",
...
async function getPhotoAnalysis(listingId: string | null, userId: string) {
  if (!listingId) {
    return NextResponse.json(
      { error: "Listing ID required for photo analysis" },
      { status: 400 }
    );
  }
...
async function getDescriptionOptimization(
  listingId: string | null,
  userId: string
) {
  if (!listingId) {
    return NextResponse.json(
      { error: "Listing ID required for description analysis" },
      { status: 400 }
    );
  }
  ...
  return [
    {
      ...
      impact: "Aktivitet i ulët",
    },
    { date: "2024-11-29", name: "Dita e Çlirimit", impact: "Aktivitet i ulët" },
    {
      date: "2024-12-25",
      name: "Krishtlindjet",
      impact: "Aktivitet shumë i ulët",
    },
  ];
}

function calculatePriceRanking(listing: any, competitors: any[]) {
  const prices = [listing.price, ...competitors.map((c) => c.price)].sort(
    (a, b) => a - b
  );
  const rank = prices.indexOf(listing.price) + 1;
  return `${rank}/${prices.length} (${
    rank <= prices.length / 3
      ? "I lirë"
      : rank <= (2 * prices.length) / 3
      ? "Mesatar"
      : "I shtrenjtë"
  })`;
}
...
function calculateMileageRanking(listing: any, competitors: any[]) {
  const mileages = [listing.mileage, ...competitors.map((c) => c.mileage)].sort(
    (a, b) => a - b
  );
  const rank = mileages.indexOf(listing.mileage) + 1;
  return `${rank}/${mileages.length} (${
    rank <= mileages.length / 3
      ? "Pak km"
      : rank <= (2 * mileages.length) / 3
      ? "Mesatar"
      : "Shumë km"
  })`;
}
...
function calculateAgeRanking(listing: any, competitors: any[]) {
  const years = [listing.year, ...competitors.map((c) => c.year)].sort(
    (a, b) => b - a
  );
  const rank = years.indexOf(listing.year) + 1;
  return `${rank}/${years.length} (${
    rank <= years.length / 3
      ? "I ri"
      : rank <= (2 * years.length) / 3
      ? "Mesatar"
      : "I vjetër"
  })`;
}
...
function generateCompetitorRecommendations(listing: any, competitors: any[]) {
  ...
  if (listing.price > avgCompetitorPrice * 1.1) {
    recommendations.push(
      "Konsideroni uljen e çmimit për të qenë më konkurrues"
    );
  }

  // Activity recommendations
  const lowActivityCompetitors = competitors.filter(
    (c) => c.activityLevel === "E ulët"
  ).length;
  ...
  return [
    {
      season: "Pranverë",
      factors: [
        "Rritje e kërkesës për cabrio",
        "Fillim i sezonit të shitjeve",
        "Aktivitet i lartë i tregut",
      ],
    },
    {
      season: "Verë",
      factors: [
        "Peak sezoni për SUV",
        "Turizëm - kërkesa për makina me qira",
        "Aktivitet maksimal",
      ],
    },
    {
...
function getPhotoRecommendations(images: any[]) {
  ...
  recommendations.push(
    "Fotografoni në dritë natyrore për rezultate më të mira"
  );
...
function analyzeKeywords(description: string, listing: any) {
  ...
  keywords.forEach((keyword) => {
    const count = (
      description.toLowerCase().match(new RegExp(keyword.toLowerCase(), "g")) ||
      []
    ).length;
...
function getMissingInformation(listing: any) {
  ...
  if (
    !listing.description.includes("document") &&
    !listing.description.includes("letra")
  ) {
    missing.push("Specifikoni statusin e dokumentave");
  }

  if (
    !listing.description.includes("shërbim") &&
    !listing.description.includes("mirëmbajtje")
  ) {
...
function calculateSEOScore(listing: any) {
  ...
  // Title optimization
  if (
    listing.title.includes(listing.make) &&
    listing.title.includes(listing.model)
  )
    score += 20;
  ...
  if (
    listing.description.includes("Shqipëri") ||
    listing.description.includes("Tiranë")
  )
    score += 10;
  if (
    listing.description.includes(listing.make) &&
    listing.description.includes(listing.model)
  )
    score += 15;

  // Location optimization
  if (["Tiranë", "Durrës", "Vlorë", "Shkodër"].includes(listing.city))
    score += 15;
...
function analyzeAlbanianLanguage(description: string) {
  ...
  const hasAlbanianWords = albanianWords.some((word) =>
